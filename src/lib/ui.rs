...
pub fn component_message(cmp: &Component, msg: String) {
    let name: String = match cmp.color {
        TerminalColor::White => format!("{}", White.bold().paint(&cmp.name)).into(),
        TerminalColor::Blue => format!("{}", Blue.bold().paint(&cmp.name)).into(),
        TerminalColor::Red => format!("{}", Red.bold().paint(&cmp.name)).into(),
        TerminalColor::Green => format!("{}", Green.bold().paint(&cmp.name)).into(),
        TerminalColor::Purple => format!("{}", Purple.bold().paint(&cmp.name)).into(),
        TerminalColor::Yellow => format!("{}", Yellow.bold().paint(&cmp.name)).into(),
    };
